...
class ModalOmnilingualSTTService(ModalWebsocketSegmentedSTTService):
    """Omnilingual ASR service for Swahili speech recognition."""
    ...
    async def _receive_messages(self):
        ...
        async for message in self._get_websocket():
            if isinstance(message, str):
                ...
                logger.info(f"Received Swahili transcription: {message}")
            else:
    ...
    async def run_stt(self, audio: bytes) -> AsyncGenerator[Frame, None]:
        ...
        if not self._websocket:
            logger.error("Not connected to Omnilingual ASR.")
            yield ErrorFrame("Not connected to Omnilingual ASR.", fatal=True)
            return
        ...
        try:
            ...
            logger.error(f"Failed to send audio to Omnilingual ASR: {e}")
            yield ErrorFrame(f"Failed to send audio to Omnilingual ASR: {e}")
            return
